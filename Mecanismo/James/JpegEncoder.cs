...
namespace F5.James
{
    ...
    public sealed class JpegEncoder : IDisposable
    {
        ...
        {
            WriteHeaders(); //Escribe los header
            WriteCompressedData();
            WriteEOI();
            this.output.Flush();
            this.output.Dispose();
        ...
        void WriteArray(byte[] data) //Escribe un marcador largo
        {
            int length = ((data[2] & 0xFF) << 8) + (data[3] & 0xFF) + 2;
            this.output.Write(data, 0, length);
        }

        void WriteMarker(byte[] data) //Escribe un marcador (2 bytes)
        {
            this.output.Write(data, 0, 2);
        }

        void WriteEOI() //Marcador de fin de la imagen
        {
            byte[] EOI = { 0xFF, 0xD9 };
            WriteMarker(EOI);
        }
        ...
        /// <summary>
        /// the SOI marker (Marcador del comienzo de la imagen)
        /// </summary>
        void WriteHeaderSOI()
        {
            byte[] SOI = { 0xFF, 0xD8 };
        ...
        /// the JFIF header
        /// </summary>
        void WriteHeaderJFIF() //Marcador con los parametros de la imagen https://en.wikipedia.org/wiki/JPEG_File_Interchange_Format
        {
        ...
        /// </summary>
        void WriteHeaderSOF0() //Marcador de inicio de frame 0
        {
            byte i;
        ...
        void WriteHeaderDHT()  //Marcador de las tablas de Huffman
        {
        ...
        void WriteHeaderSOS() //Marcador de Start of Scan
        {
            byte i;
        ...
        void WriteHeaders()
        {
            WriteHeaderSOI(); //Comienzo de la imagen
            WriteHeaderJFIF(); //Parametros de la imagen
            WriteHeaderComment(); //Comentario
            WriteHeaderDQT(); //Tablas de cuantificacion
            WriteHeaderSOF0(); //Inicio de frame 0 (Start of frame 0)
            WriteHeaderDHT(); //Tablas de Huffman
            WriteHeaderSOS(); //Strart of Scan
        }
